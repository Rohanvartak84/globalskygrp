...
const FOUNDERS = [
  ...
  { name: 'Rucha Joshi', role: 'Co-Founder & Operations', image: 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?auto=format&fit=crop&w=700&q=80' },
];
    ...
    <main className="bg-white text-slate-900 overflow-x-hidden">
      ...
      <section className="relative min-h-[72svh] flex items-center overflow-hidden bg-[#061d44] pt-28">
        ...
        <div className="container relative z-10 py-16 lg:py-24">
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="font-display text-[clamp(2.2rem,6vw,4.2rem)] leading-[1.05] text-white font-bold max-w-4xl">About Global Sky Group</motion.h1>
        ...
        <div className="container">
          <div className="rounded-3xl border border-white/25 bg-white/55 backdrop-blur-xl p-5 sm:p-8 lg:p-10 shadow-premium">
            <div className="grid gap-8 lg:grid-cols-2">
              <div>
                <h2 className="font-display text-3xl sm:text-4xl font-bold text-brand-blue">About Us</h2>
                <p className="mt-4 text-slate-700 leading-relaxed">Welcome to Let Me Fly Holidays — your trusted travel partner in creating unforgettable vacation experiences! We understand that planning the perfect getaway can be overwhelming, so we're here to simplify the process and make it as enjoyable as possible.</p>
                <h3 className="mt-8 font-display text-2xl font-bold text-brand-blue">Vision</h3>
                <p className="mt-3 text-slate-700 leading-relaxed">Our vision is to be the most trusted and innovative travel agency, recognized for creating exceptional and personalized travel experiences that inspire exploration and create lasting memories.</p>
                <h3 className="mt-8 font-display text-2xl font-bold text-brand-blue">Mission</h3>
                <ul className="mt-3 space-y-2.5">
                  {['To guide customers responsibly', 'To deliver accurate and honest travel advice', 'To build lifelong trust', 'To grow sustainably through integrity'].map((point) => (
              ...
              <div>
                <h2 className="font-display text-3xl sm:text-4xl font-bold text-brand-blue mb-5">Our Founders</h2>
      ...
      <section className="py-16 lg:py-24">
        <div className="container">
          <h2 className="font-display text-3xl lg:text-5xl font-bold text-slate-900 text-center">Why Choose Us</h2>
        ...
        <div className="container grid grid-cols-2 lg:grid-cols-4 gap-4">
          {STATS.map((s) => (
            <div key={s.label} className="rounded-2xl border border-white/15 bg-white/10 backdrop-blur text-center p-5">
              <div className="font-display text-3xl lg:text-4xl text-brand-orange font-bold">{s.value}</div>
      ...
      <section className="relative py-16 lg:py-24 overflow-hidden">
        <div className="absolute inset-0">
          <img src="/hero-skyline.jpg" alt="" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-cta-overlay" />
        </div>
        <div className="container relative z-10 text-center text-white">
          <Target className="mx-auto w-10 h-10 text-brand-orange" />
          <h2 className="mt-4 font-display text-3xl lg:text-5xl font-bold">Ready to Plan Your Next Journey?</h2>
          ...
          <div className="mt-7 flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Button asChild variant="outline" className="bg-white/10 border-white/30 text-white rounded-full px-7 py-6"><a href="https://wa.me/918140840069" target="_blank" rel="noreferrer"><MessageCircle className="mr-1.5 w-4 h-4" /> Chat on WhatsApp</a></Button>
          </div>
        </div>
      </section>
